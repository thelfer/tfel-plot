...
namespace tfel
{
  ...
  namespace plot
  {
    ...
    GnuplotInterpreter::KrigingInterpreter::KrigingInterpreter(GnuplotInterpreter& i,
							       Graph& graph)
    ...
    void
    GnuplotInterpreter::KrigingInterpreter::eval(const_iterator& p, 
						 const const_iterator pe)
    {
      ...
      QVector<double> vx4;
      QVector<double> vy;
      QVector<double>::const_iterator pt;
      QVector<double>::const_iterator pt2;
      QVector<double>::const_iterator pt3;
      QVector<double>::const_iterator pt4;
      ...
      if(numberOfVariables==-1){
    	...
    	if(p->value=="("){
    	  const QVector<string>& vars = this->readVariableList(p,pe);
    	...
    	if(p!=pe){
    	  if(p->value=="("){
    	    const QVector<std::string>& vars = this->readVariableList(p,pe);
      ...
      if(p==pe){
    	...
    	// using 
    	CxxTokenizer::readSpecifiedToken("GnuplotInterpreter::treatKriging : ",
    					 "using",p,pe);
    	CxxTokenizer::checkNotEndOfLine("GnuplotInterpreter::treatKriging","",p,pe);
    	if(varNumber==1){
      ...
      if(varNumber==1){
    	...
    	interpreter.addFunction(function,EFunctionPtr(new KrigedFunction<1>(points)),
				false,false);
      } else if(varNumber==2){
    	...
    	interpreter.addFunction(function,EFunctionPtr(new KrigedFunction<2>(points)),
				false,false);
      } else if(varNumber==3){
    	if((vx1.size()!=vx2.size())||
    	   (vx1.size()!=vx3.size())||
    	...
    	interpreter.addFunction(function,EFunctionPtr(new KrigedFunction<3>(points)),
				false,false);
      } else {
    	string msg("GnuplotInterpreter::treatKriging : ");
    	msg += "unsupported number of variables";
