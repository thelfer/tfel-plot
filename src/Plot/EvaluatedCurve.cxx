...
namespace tfel {
  ...
  namespace plot {
    ...
    EvaluatedCurve::EvaluatedCurve(std::shared_ptr<tfel::math::Evaluator>& f,
                                   const QString& v)
        : ev(f), hasSamples(false), isConstant(false) {
      using namespace std;
    ...
    void EvaluatedCurve::setNumberOfSamples(const unsigned short f,
                                            const bool b) {
    ...
    unsigned short EvaluatedCurve::getNumberOfSamples() const {
      return this->samples;
