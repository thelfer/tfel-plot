...
namespace tfel {
  ...
  namespace plot {
    ...
    struct ElegantGraphTheme : public StandardGraphTheme {
      void getGraphBorders(
          Graph&, qreal& gl, qreal& gr, qreal& gu, qreal& gd) const override {
        gl = 6;
        gr = 16;
        gu = 6;
        gd = 16;
      ...
      void drawGraphBackGround(Graph& g,
                               const GraphLayout&,
                               const GraphSize& s) const override {
        auto& cr = g.getScene();
        cr.addRect(s.xh0, s.yh0, s.xh1 - s.xh0, s.yh1 - s.yh0, QPen(),
                   Qt::white);
      }  // end of drawBackGround
      ...
      QColor getTextColor() const { return Qt::red; }  // end of getTextColor

      virtual void drawHorizontalBox(QGraphicsScene& cr,
                                     const qreal x,
                                     const qreal y,
                                     const qreal w,
                                     const qreal h) const {
      ...
      virtual void drawVerticalBox(QGraphicsScene& cr,
                                   const qreal x,
                                   const qreal y,
                                   const qreal w,
                                   const qreal h) const {
      ...
      void printXLabel(Graph& g,
                       const GraphLayout& l,
                       const GraphSize& s,
                       const QString& xlabel) const override {
        QGraphicsScene& cr = g.getScene();
        QGraphicsTextItem* t = new GraphTextItem(xlabel, &g, &Graph::setXLabel);
        const QRectF b = t->boundingRect();
        this->drawHorizontalBox(
            cr, s.xh0 + 0.5 * (s.xh1 - s.xh0) - 0.5 * b.width(),
            s.height - l.md - l.ttd - l.ld, b.width(), l.ld);
    ...
    StandardThemeProxy<ElegantGraphTheme> elegantGraphThemeProxy(
        "ElegantGraphTheme");
