...
namespace tfel {
  ...
  namespace plot {
    ...
    struct GnuplotInterpreter::ImportInterpreter::ImportOptions {
      enum ImportType { Function, Data, Default };
      enum FunctionType { C, Castem, Cyrano, FDefault };
    ...
    GnuplotInterpreter::ImportInterpreter::ImportInterpreter(
    ...
    void GnuplotInterpreter::ImportInterpreter::treatImport(
        const_iterator& p, const const_iterator pe, const bool b) {
      auto throw_if = [](const bool c, const std::string& m) {
        tfel::raise_if(
            c,
            "GnuplotInterpreter::ImportInterpreter::treatImport: " + m);
      };
      ImportOptions options;
      CxxTokenizer::checkNotEndOfLine(
          "GnuplotInterpreter::ImportInterpreter::treatImport : ",
      ...
      if (p->value == "<") {
        ...
        while (p->value != ">") {
          ...
          } else if (p->value == "cyrano") {
            throw_if(!((options.type == ImportOptions::Default) ||
                       (options.type == ImportOptions::Function)),
                     "import type is neither 'Default' nor 'Function', "
                     "which is required for the 'cyrano' option");
            throw_if(options.functionType != ImportOptions::FDefault,
                     "a function type has already been specified");
            options.type = ImportOptions::Function;
            options.functionType = ImportOptions::Cyrano;
    ...
    void GnuplotInterpreter::ImportInterpreter::importCastemFunction(
        const std::string& function,
        const std::string& library,
        const unsigned short varNumber,
        const bool b) {
      auto& elm = tfel::system::ExternalLibraryManager::
          getExternalLibraryManager();
      const auto nb =
          elm.getCastemFunctionNumberOfVariables(library, function);
      tfel::raise_if(nb != varNumber,
                     "GnuplotInterpreter::ImportInterpreter::"
                     "importCastemFunction: "
                     "the function '" +
                         function + "' declares " + std::to_string(nb) +
                         " variables, not " +
                         std::to_string(varNumber) + " as requested");
      auto func = elm.getCastemFunction(library, function);
      interpreter.addFunction(
          function,
          std::make_shared<tfel::math::parser::ExternalCastemFunction>(
              func, varNumber),
          b, false);
    }  // end of
       // GnuplotInterpreter::ImportInterpreter::importCastemFunction

    void GnuplotInterpreter::ImportInterpreter::importCyranoFunction(
        const std::string& function,
        const std::string& library,
        const unsigned short varNumber,
        const bool b) {
      auto& elm = tfel::system::ExternalLibraryManager::
          getExternalLibraryManager();
      const auto nb =
          elm.getCyranoMaterialPropertyNumberOfVariables(library, function);
      tfel::raise_if(nb != varNumber,
                     "GnuplotInterpreter::ImportInterpreter::"
                     "importCyranoFunction: "
                     "the function '" +
                         function + "' declares " + std::to_string(nb) +
                         " variables, not " +
                         std::to_string(varNumber) + " as requested");
      auto func = elm.getCyranoMaterialProperty(library, function);
      interpreter.addFunction(
          function,
          std::make_shared<tfel::math::parser::ExternalCyranoFunction>(
              func, varNumber),
          b, false);
    }  // end of
       // GnuplotInterpreter::ImportInterpreter::importCyranoFunction

    void GnuplotInterpreter::ImportInterpreter::importCFunction(
        const std::string& f,
        const std::string& l,
        const unsigned short varNumber,
        const bool b) {
      ...
      switch (varNumber) {
        case 0:
          declare(std::make_shared<ExternalCFunction<0>>(
              elm.getCFunction0(l, f)));
          break;
        case 1:
          declare(std::make_shared<ExternalCFunction<1>>(
              elm.getCFunction1(l, f)));
          break;
        case 2:
          declare(std::make_shared<ExternalCFunction<2>>(
              elm.getCFunction2(l, f)));
          break;
        case 3:
          declare(std::make_shared<ExternalCFunction<3>>(
              elm.getCFunction3(l, f)));
          break;
        case 4:
          declare(std::make_shared<ExternalCFunction<4>>(
              elm.getCFunction4(l, f)));
          break;
        case 5:
          declare(std::make_shared<ExternalCFunction<5>>(
              elm.getCFunction5(l, f)));
          break;
        case 6:
          declare(std::make_shared<ExternalCFunction<6>>(
              elm.getCFunction6(l, f)));
          break;
        case 7:
          declare(std::make_shared<ExternalCFunction<7>>(
              elm.getCFunction7(l, f)));
          break;
        case 8:
          declare(std::make_shared<ExternalCFunction<8>>(
              elm.getCFunction8(l, f)));
          break;
        case 9:
          declare(std::make_shared<ExternalCFunction<9>>(
              elm.getCFunction9(l, f)));
          break;
        case 10:
          declare(std::make_shared<ExternalCFunction<10>>(
              elm.getCFunction10(l, f)));
          break;
        case 11:
          declare(std::make_shared<ExternalCFunction<11>>(
              elm.getCFunction11(l, f)));
          break;
        case 12:
          declare(std::make_shared<ExternalCFunction<12>>(
              elm.getCFunction12(l, f)));
          break;
        case 13:
          declare(std::make_shared<ExternalCFunction<13>>(
              elm.getCFunction13(l, f)));
          break;
        case 14:
          declare(std::make_shared<ExternalCFunction<14>>(
              elm.getCFunction14(l, f)));
          break;
        case 15:
          declare(std::make_shared<ExternalCFunction<15>>(
              elm.getCFunction15(l, f)));
          break;
        default:
          tfel::raise(
              "GnuplotInterpreter::ImportInterpreter::importCFunction: "
              "function with more than 15 variables are not allowed.");
      }
    }  // end of GnuplotInterpreter::ImportInterpreter::importCFunction

    void GnuplotInterpreter::ImportInterpreter::importFunction(
        const_iterator& p,
        const const_iterator pe,
        const GnuplotInterpreter::ImportInterpreter::ImportOptions
            options,
        const bool b) {
      CxxTokenizer::checkNotEndOfLine(
      ...
      if (options.numberOfVariables == -1) {
        ...
        if (p != pe) {
          if (p->value == "(") {
            const auto& vars = this->readVariableList(p, pe);
            tfel::raise_if(
                vars.size() != static_cast<unsigned short>(
                                   options.numberOfVariables),
                "GnuplotInterpreter::ImportInterpreter::importFunction:"
                " "
                "the number variables of function '" +
                    function +
                    "'"
        ...
        varNumber =
            static_cast<unsigned short>(options.numberOfVariables);
      }
      CxxTokenizer::checkNotEndOfLine(
          "GnuplotInterpreter::ImportInterpreter::importFunction : ",
      ...
      } else if (options.functionType == ImportOptions::Cyrano) {
        this->importCyranoFunction(function, library, varNumber, b);
      } else {
        tfel::raise(
            "GnuplotInterpreter::ImportInterpreter::importFunction: "
