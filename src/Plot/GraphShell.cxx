...
namespace tfel{
  ...
  namespace plot{
	...
	if(np>0){
	  ...
	  c.movePosition(QTextCursor::NextCharacter,
			 QTextCursor::KeepAnchor,np);
	  QClipboard *clipboard = QApplication::clipboard();
	  clipboard->setText(n.selectedText(),QClipboard::Selection);
	} else {
	  n.movePosition(QTextCursor::NextCharacter,
			 QTextCursor::MoveAnchor,0);
	  n.movePosition(QTextCursor::NextCharacter,
			 QTextCursor::KeepAnchor,-np);
	  QClipboard *clipboard = QApplication::clipboard();
	...
	this->setTextColor(this->getUserInputTextColor());
      ...
      if((this->yank)&&((e->modifiers() == Qt::ControlModifier)&&(e->key()!=Qt::Key_Y))&&
	 ((e->modifiers() == Qt::AltModifier)&&(e->key()==Qt::Key_Y))){
	yank = false;
      }
      if(((e->modifiers() == Qt::ControlModifier)&&(e->key()==Qt::Key_X))&&(!ctrlx)){
	ctrlx = true;
	return;
      }
      if(this->ctrlx){
	...
	this->setTextColor(this->getUserInputTextColor());
		...
		((e->modifiers() == Qt::ControlModifier)&&(e->key()==Qt::Key_B))||
	...
	this->moveCursor(QTextCursor::NextCharacter,
		...
		((e->modifiers() == Qt::ControlModifier)&&(e->key()==Qt::Key_P))||
		(e->key()==Qt::Key_PageUp)){
	this->searchHistoryBackWard();
      } else if((e->key()==Qt::Key_Down)||
		((e->modifiers() == Qt::ControlModifier)&&(e->key()==Qt::Key_N))||
		(e->key()==Qt::Key_PageDown)){
	this->searchHistoryUpWard();
      } else if((e->modifiers() == Qt::AltModifier)&&(e->key()==Qt::Key_Y)){
	...
	c = this->textCursor();
	c.movePosition(QTextCursor::PreviousCharacter,QTextCursor::KeepAnchor,cps);
	c.removeSelectedText();
	this->setTextColor(this->getUserInputTextColor());
	c.insertText(this->ring[this->pring]);
	this->setTextCursor(c);
      } else if((e->modifiers() == Qt::AltModifier)&&(e->key()==Qt::Key_W)){
	this->addToKillRing(this->textCursor().selectedText());
      } else if((e->modifiers() == Qt::AltModifier)&&(e->key()==Qt::Key_B)){
	if(!(posInLine<=ps)){
      ...
      } else if((e->modifiers() == Qt::AltModifier)&&(e->key()==Qt::Key_D)){
	 c = this->textCursor();
	c.movePosition(QTextCursor::NoMove,
		       QTextCursor::MoveAnchor);
