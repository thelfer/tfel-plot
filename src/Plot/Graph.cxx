...
namespace tfel
{
  ...
  namespace plot
  {
    ...
    QColor
    Graph::getColorByName(const QString& n)
    {
      if(n=="white"){
	return Qt::white;
      } else if(n=="black"){
    ...
    {
      this->reset();
      this->setTheme(ThemeManager::getThemeManager().getTheme("StandardGraphTheme"),false);
    ...
    int
    Graph::heightForWidth(int w) const{
    ...
    Graph::getCurves() const{
      auto c = QVector<std::shared_ptr<Curve>>{};
      for(const auto& ch : this->curves){
    ...
    {
      if(w<=0){
	throw(std::runtime_error("Graph::setGraphWidth: "
    ...
    {
      if(h<=0){
	throw(std::runtime_error("Graph::setWidth: "
				 "negative or null width specified"));
      }
      this->height = h;
	...
	if(ch.axis&axis.id){
	  ...
	  for(const auto& pt : ch.points){
		...
		QMessageBox::critical(this,tr("Graph::setLogScale"),
				      tr("negative value detected  (%1)"
					 "for an untitled curve").arg(value));
		return;
	      }
	    }
	  }
    ...
    void Graph::getRange(qreal& r0,qreal& r1,
	...
	throw(std::runtime_error("Graph::getRange: "
	...
	break;
      default:
	throw(std::runtime_error("Graph::convertHorizontalGraphCoordinatesToAbsolutePosition: "
	...
	break;
      default:
	throw(std::runtime_error("Graph::convertVerticalGraphCoordinatesToAbsolutePosition: "
				 "unsupported coordinate type"));
	break;
      }
      return yp;
    ...
    Graph::~Graph()
    {
      this->removeCurves(false);
    } // end of Graph::~Graph()

    qreal
    Graph::computeTicsIncrements(const qreal min,
    ...
    QString
    Graph::convertTicsToString(const qreal d,
			      ...
			      GraphLayout& l)
    {
      ...
      // graph borders
      this->theme->getGraphBorders(*this,
				   l.gl,l.gr,
      				   l.gu,l.gd);
      // summing-up
		     ...
		     const unsigned short pheight)
    {
      ...
      GraphSize s;
      GraphLayout l;
      QVector<CurveHandler>::iterator p;
      QVector<shared_ptr<Label> >::const_iterator p3;
      QVector<shared_ptr<Arrow> >::const_iterator p4;
      unsigned short nbr;
	...
	this->theme->printY2Tics(*this,l,s,this->y2Axis.tics);
      ...
      // draw curves
      nbr = 1u;
      auto* d = this->scene->addRect(s.xh0,s.yh0,s.xh1-s.xh0,s.yh1-s.yh0);
      d->setPen(Qt::NoPen);
      d->setFlags(QGraphicsItem::ItemClipsChildrenToShape);
      for(p=this->curves.begin();p!=this->curves.end();++p,++nbr){
      	if(p->axis==Graph::XY){
      	  this->theme->drawCurve(*this,*d,p->curve.get(),p->points,s,
      				 l.ax,l.bx,l.ay,l.by,nbr);
      	} else if(p->axis==Graph::X2Y){
      	  this->theme->drawCurve(*this,*d,p->curve.get(),p->points,s,
      				 l.ax2,l.bx2,l.ay,l.by,nbr);
      	} else if(p->axis==Graph::XY2){
      	  this->theme->drawCurve(*this,*d,p->curve.get(),p->points,s,
      				 l.ax,l.bx,l.ay2,l.by2,nbr);
      	} else {
      	  this->theme->drawCurve(*this,*d,p->curve.get(),p->points,s,
      				 l.ax2,l.bx2,l.ay2,l.by2,nbr);
      	}
      }
    
      this->theme->drawGrid(*this,l,s,this->showGraphGrid,this->grid,
      ...
      for(p3=this->labels.begin();p3!=this->labels.end();++p3){
      	const Label& pl = *(*p3);
      	this->theme->printLabel(*this,this->convertHorizontalGraphCoordinatesToAbsolutePosition(l,pl.x),
      ...
      for(p4=this->arrows.begin();p4!=this->arrows.end();++p4){
      	const Arrow& pa = *(*p4);
      	this->theme->drawArrow(*this,this->convertHorizontalGraphCoordinatesToAbsolutePosition(l,pa.x0),
	...
	this->updateRange(this->xAxis,this->xAxis.min,
			  this->xAxis.max);
      }
      if(b){
	this->replot();
	...
	this->updateRange(this->x2Axis,this->x2Axis.min,
			  this->x2Axis.max);
      }
      if(b){
	this->replot();
    ...
    void Graph::updateCurve(CurveHandler& c,
			    const Graph::Axis& axis)
    {
      if(axis.scale==Graph::Axis::LOGSCALE){
	this->computePoints(c,std::pow(10.,axis.min),std::pow(10.,axis.max));
      } else {
    ...
    bool
    Graph::computeMinMax(qreal& fv0,
			 qreal& fv1,
			 Graph::Axis& axis)
    {
      qreal v0 =  std::numeric_limits<qreal>::max();
      qreal v1 = -std::numeric_limits<qreal>::max();
      const bool abscissa = (axis.id&Graph::xaxis)||(axis.id&Graph::x2axis);
      bool found = false;
      for(const auto& c : this->curves){
	if(c.axis&axis.id){
	  if(!c.points.empty()){
	...
	*p = std::make_shared<Label>(name,label,xc,yc);
	...
	this->arrows.push_back(std::make_shared<Arrow>(name,x0,y0,x1,y1,s));
      } else {
	*p = std::make_shared<Arrow>(name,x0,y0,x1,y1,s);
    ...
    QVector<std::shared_ptr<Graph::Label> >::iterator
    Graph::findLabel(const QString& n)
    {
      return std::find_if(this->labels.begin(),this->labels.end(),
			  [&n](const std::shared_ptr<Label>& p){
			    return p->name==n;});
    } // end of Graph::findLabel

    QVector<std::shared_ptr<Graph::Arrow> >::iterator
    Graph::findArrow(const QString& n)
    {
      for(auto p=this->arrows.begin();
	  p!=this->arrows.end();++p){
	if((*p)->name==n){
	  return p;
	}
		       ...
		       const bool b)
    {
      ...
      if(p==this->labels.end()){
	throw(std::runtime_error("Graph::removeLabel: "
				 "unknown label '"+n.toStdString()+"'"));
      }
      this->labels.erase(p);
      if(b){
	this->replot();
		       ...
		       const bool b)
    {
      ...
      if(p==this->arrows.end()){
	throw(std::runtime_error("Graph::removeArrow: "
				 "unknown arrow '"+n.toStdString()+"'"));
      }
      this->arrows.erase(p);
      if(b){
	this->replot();
	...
	if(ev->button()==Qt::LeftButton){
	  ...
	  this->rubberBandRect = QRect(pev.x(),pev.y(),
				       0.,0.);
    ...
    QVector<Graph::CurveHandler>::iterator
    Graph::findCurveHandler(Curve *const c)
    {
      return std::find_if(this->curves.begin(),this->curves.end(),[c](const CurveHandler& ch){
	  return ch.curve.get()==c;
	});
