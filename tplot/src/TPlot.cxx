...
namespace tfel {
  ...
  namespace plot {
    ...
    TPlot::CurveOptions& TPlot::Input::getCurveOptions() {
      ...
      throw(
          std::runtime_error("TPlot::Input::getCurveOptions: "
    ...
    unsigned short TPlot::convertToUnsignedShort(const QString& qvalue) {
      ...
      for (const auto& c : value) {
        if (!isdigit(c)) {
          throw(std::runtime_error(
              "TPlot::convertToUnsignedShort: invalid entry"));
        }
      }
      ...
      if (!converter && (!converter.eof())) {
        throw(
            std::runtime_error("TPlot::convertToUnsignedShort: "
    ...
    TPlot::TPlot(const int argc, const char* const* const argv)
    ...
    struct ChooseImportFilter : public QDialog {
      ChooseImportFilter(QWidget* p) : QDialog(p) {
        ...
        QObject::connect(bbox, &QDialogButtonBox::accepted, this,
                         &ChooseImportFilter::accept);
      }
      int getFilterId() { return this->bgrp->checkedId(); }
    ...
    void TPlot::createActions() {
      ...
      QObject::connect(this->epdfa, &QAction::triggered, this,
                       &TPlot::exportToPDF);
      ...
      QObject::connect(this->esvga, &QAction::triggered, this,
                       &TPlot::exportToSVG);
      this->epnga = new QAction(tr("Export to png"), this);
      this->epnga->setStatusTip(
          tr("Export the graph in Portable Network Plot format"));
      this->epnga->setIcon(QIcon::fromTheme("document-save-as"));
      this->epnga->setIconVisibleInMenu(true);
      QObject::connect(this->epnga, &QAction::triggered, this,
                       &TPlot::exportToPNG);
      ...
      QObject::connect(this->xloga, &QAction::triggered, this->g,
                       &Graph::setXAxisLogScale);
      this->x2loga = new QAction(tr("set logscale"), this);
      QObject::connect(this->x2loga, &QAction::triggered, this->g,
                       &Graph::setX2AxisLogScale);
      this->yloga = new QAction(tr("set logscale"), this);
      QObject::connect(this->yloga, &QAction::triggered, this->g,
                       &Graph::setYAxisLogScale);
      this->y2loga = new QAction(tr("set logscale"), this);
      QObject::connect(this->y2loga, &QAction::triggered, this->g,
                       &Graph::setY2AxisLogScale);
      this->xuloga = new QAction(tr("unset logscale"), this);
      QObject::connect(this->xuloga, &QAction::triggered, this->g,
                       &Graph::unsetXAxisLogScale);
      this->x2uloga = new QAction(tr("unset logscale"), this);
      QObject::connect(this->x2uloga, &QAction::triggered, this->g,
                       &Graph::unsetX2AxisLogScale);
      this->yuloga = new QAction(tr("unset logscale"), this);
      QObject::connect(this->yuloga, &QAction::triggered, this->g,
                       &Graph::unsetYAxisLogScale);
      this->y2uloga = new QAction(tr("unset logscale"), this);
      QObject::connect(this->y2uloga, &QAction::triggered, this->g,
                       &Graph::unsetY2AxisLogScale);
      ...
      connect(this->cba, &QAction::triggered, this,
              &TPlot::copyToClipboardAsBitmap);
    ...
    void TPlot::registerArgumentCallBacks() {
      ...
      this->registerNewCallBack("--with-dashdotdotline",
                                ...
                                "draw the preceeding curve with a "
                                "succession of dash and two dots");
      this->registerNewCallBack(
          "--with-lineplus", &TPlot::treatWithLinePlus,
    ...
    bool TPlot::graphicalOutput() const { return this->goutput; }
    ...
    QString TPlot::getStringOption(const bool b) {
      ...
      if (!b) {
        if (o.empty()) {
          throw(std::runtime_error(
              "TPlot::getStringOption: no argument given to the '" +
    ...
    double TPlot::getDoubleOption() {
      ...
      throw_if(o.empty(), "no argument given to the '" +
                              std::string{*(this->currentArgument)} +
                              "' option");
      bool b;
    ...
    void TPlot::treatInputSocket() {
      auto throw_if = [](const bool b, const std::string& m) {
        if (b) {
          throw(std::runtime_error("TPlot::treatInputSocket: " + m));
        }
      };
      const auto o = this->currentArgument->getOption();
      throw_if(o.empty(), "no argument given to the '" +
    ...
    void TPlot::treatOutputSocket() {
      auto throw_if = [](const bool b, const std::string& m) {
        if (b) {
          throw(std::runtime_error("TPlot::treatOutputSocket: " + m));
        }
      };
      const auto o = this->currentArgument->getOption();
      throw_if(o.empty(), "no argument given to the '" +
    ...
    void TPlot::treatXY() {
      if (this->inputs.empty()) {
        throw(
            std::runtime_error("TPlot::treatXY: "
                               "no curve specified"));
      }
      auto& o = this->inputs.back().getCurveOptions();
      o.axis = Graph::XY;
    ...
    void TPlot::treatX2Y() {
      if (this->inputs.empty()) {
        throw(
            std::runtime_error("TPlot::treatX2Y: "
                               "no curve specified"));
      }
      auto& o = this->inputs.back().getCurveOptions();
      o.axis = Graph::X2Y;
    ...
    void TPlot::treatXY2() {
      if (this->inputs.empty()) {
        throw(
            std::runtime_error("TPlot::treatXY2: "
                               "no curve specified"));
      }
      auto& o = this->inputs.back().getCurveOptions();
      o.axis = Graph::XY2;
    ...
    void TPlot::treatX2Y2() {
      if (this->inputs.empty()) {
        throw(
            std::runtime_error("TPlot::treatX2Y2: "
                               "no curve specified"));
      }
      auto& o = this->inputs.back().getCurveOptions();
      o.axis = Graph::X2Y2;
    ...
    void TPlot::treatUpperTitle() {
      this->getStringFromArgs(this->upperTitle, "treatUpperTitle",
                              "upperTitle");
    ...
    void TPlot::treatRightTitle() {
      this->getStringFromArgs(this->rightTitle, "treatRightTitle",
                              "rightTitle");
    }  // end of TPlot::treatRightTitle

    void TPlot::treatUpperLabel() {
      this->getStringFromArgs(this->upperLabel, "treatUpperLabel",
                              "upperLabel");
    ...
    void TPlot::treatRightLabel() {
      this->getStringFromArgs(this->rightLabel, "treatRightLabel",
                              "rightLabel");
    ...
    void TPlot::treatKeyHorizontalPosition() {
      this->getStringFromArgs(this->keyHorizontalPosition,
                              "treatKeyHorizontalPosition",
    ...
    void TPlot::treatColor() {
      if (this->inputs.empty()) {
        throw(
            std::runtime_error("TPlot::treatColor: "
                               "no curve specified"));
      }
    ...
    void TPlot::treatUsing() {
      ...
      throw_if(!this->inputs.back().is<Data>(),
               "last input is not a data curve");
      const auto c =
          TPlot::tokenize(this->getStringOption().toStdString(), ':');
      if (c.size() == 1) {
      ...
      auto import_data = [this, throw_if](const QString& f, const QString& s) {
        ...
        if (pd == this->dataSources.end()) {
          ...
          throw_if(pd->second->getSeparator() != s,
                   "inconsistent separator specificed for file '" +
                       f.toStdString() + "'");
        }
        Input i;
        i.set(d);
    ...
    std::string TPlot::getVersionDescription() const { return "1.0"; }
    ...
    void TPlot::insertImageFromFile() {
      ...
      const auto f = QFileDialog::getOpenFileName(
          this, tr("Open Image"), "", tr("Image Files (%1)").arg(e));
      if (!f.isEmpty()) {
    ...
    void TPlot::selectFontDialog() {
      bool ok;
      auto f = QFontDialog::getFont(&ok, this->g->getGraphFont(), this,
                                    "Select graph font");
      if (ok) {
